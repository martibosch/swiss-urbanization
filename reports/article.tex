...
% The SFSO inventory is stored within a relational database where each entry corresponds to the coordinates of a given pixel and multiple columns that feature its respective land use/land cover classes for each survey period.
% In order to process the data in an automated and reproducible manner, an open source reusable toolbox to manage, transform and export categorical raster maps has been developed in Python \cite{bosch2019swisslandstats}.
...
% % Although th
% In view of the fractal (hence self-similar) traits observed over the three urban agglomerations, and considering the urban agglomerations of Bern and Lausanne have almost identical population size, it is quite natural to ask to what extent the distinctive spatiotemporal patterns of urbanization observed in Zurich might be attributable to its larger population size, which is more than three times as large.
...
% Such allometric perspectives are central to understand the potential universality of the processes driving urbanization \cite{bettencourt2007growth,batty2008size}, and therefore whether urban landscapes can be expected to globally become more homogeneous \cite{jenerette2010global}.
% % However, such matter cannot be properly addressed in the present study due to its limited scope.
% However, due to its limited scope, the present study is not well suited to address such matters.
% The only related observation is that the largest urban agglomeration, i.e., Zurich, is the most dense and shows the higher PLAND, shape complexity (as indicated by ED, AWMFD and LSI) and CONTAG, which suggest that despite the higher abundance of urban patches, they refuse to coalesce.
...
% As reviewed by White et al. \cite{white2015modeling}, urban patches are expected to grow as cities and regions grow, which suggests that eventually urban patches coalesce and thus the small ones disappear. Yet this contrasts with the continuous fragmentation and the persistence of the power-law scaling of the urban patches observed in the present study.
% As urbanization unfolds, urban patches are expected to grow, which suggests that eventually urban patches coalesce and thus the small ones disappear. Yet this contrasts with the continuous fragmentation and the persistence of the power-law scaling of the urban patches observed in the present study.
...
% % TODO: 1/f noise
% The seminal work of White and Engelen \cite{white1993cellular} experimented with various cellular automata models of land use change and showed that for an appropriate level of stochastic perturbation, the simulated cities quickly self-organize into a fractal structure where the size distribution of urban patches exhibits a power-law. Furthermore, their experiments showed that such structure emerged robustly despite very different initial conditions, and was maintained throughout the simulation.
...
Nevertheless, convincingly linking the observed urban patterns to models that account for socioeconomic, cultural and political drivers of urbanization remains a challenge \cite{batty2001modelling, manson2006complexity}. % many models \cite{batty2001modelling}
...
% Although new patches naturally emerge in cities and regions (e.g., new residential developments in rural areas or new commercial clusters in residential areas), it remains surprising that they appear at a rate that maintains the size-frequency distribution of urban patches as a power-law over the whole study period.
% Although new patches naturally emerge in cities and regions (e.g., new residential developments in rural areas or new commercial clusters in residential areas), it remains surprising that they appear at a rate that maintains the size-frequency distribution of urban patches as a power-law \cite{white2015modeling}.

% Potential explanations for such behavior --- which has been observed for the three agglomerations over the whole period considered in the present study --- are manifold.
The continuous fragmentation and persistence of the power-law scaling reported above might result from multiple factors.
% On the one hand, the distribution of nodal connections in transportation networks of contemporary cities tend to follow a power-law, reflecting a hierarchical organization with a few major arterial roads, a medium number of mid-sized collector streets and a large number of local streets \cite{batty2008size, boeing2018measuring}.
...
% The organization of these networks sets the pace of the dynamic processes that occur upon them
% The organization of these networks, which is akin to that found in a vast variety of biological organisms, sets the pace of the dynamic processes that occur upon them ; many topological features of street networks are consistent with fractal order  \cite{bettencourt2007growth}
% Such organization exerts a central influence in the flows of people, goods and information that underpin the spatial development of cities and regions,  \cite{jiang2009street}
...
% The way in which exploration is observed to be more active in earlier stages and is followed by a phase of densification is seemingly related to the diffusion and coalescence hypothesis as well as to the increasing dominance of infilling.
