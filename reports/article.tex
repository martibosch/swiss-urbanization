...
This paper exploits three different frameworks, namely landscape metrics, urban growth modes and fractal analysis to characterize the spatiotemporal patterns of urbanization of the Swiss urban agglomerations of Zurich, Bern and Lausanne. % from 1980 to 2016
...
The CLC inventory comprises a hierarchical nomenclature of 3 levels, where level 3 comprises 44 land cover classes, and datasets have been released for 1990, 2000, 2006, 2012 and 2018, with a spatial resolution of one hectare per pixel. % \footnote{Although Switzerland was not part of the first CLC inventory for the year 1990, Switzerland published in 1998 a compatible dataset for CLC 1990 \cite{steinmeier2013corine}. By the release of the following CLC dataset in 2000, Switzerland was already a member of the CORINE project, therefore the CLC datasets for 2000, 2006, 2012 and 2018 already include the country.}.
% The SLS land use land cover maps are assembled by the Swiss Federal Statistical Office (SFSO) as part of the production process of the national maps 
...
% principle of predominant use, which tends to systematically underpredict the human impacts by completely eliminating human features that occupy modest parts
% interpretation by sampling
...
\begin{figure}[!ht]
  \begin{adjustwidth}{-.4\textwidth}{0cm}
    \centering  
    \includegraphics[width=\linewidth]{figures/sls_clc_comparison.pdf}
    \vspace{.5em}
...
% The preparation of the extracts for the urban agglomerations of Zurich, Bern and Lausanne
...
% The SLS inventory is managed within a relational database where each entry corresponds to the coordinates of a given pixel and its respective LULC class for each survey period\footref{fn:sls-years}. % which can be downloaded in a comma-separated value text file.
